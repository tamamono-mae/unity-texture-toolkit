...
$resourceToExport = [
  ...
  'icon'=>[
    ...
    [ 'bundleNameMatch'=>'/^a\/icon_icon_emblem_\d+\.unity3d$/',     'nameMatch'=>'/^icon_emblem_(\d+)$/i',     'exportTo'=>'icon/emblem/$1', 'extraParam'=>'-s 365x64' ], //274x48 in game, 228x40 in card
  ],/*
  'unit'=>[
...
define('RESOURCE_PATH_PREFIX', '../web/redive_tw101/');
  ...
  foreach ($manifest as $name => $info) {
    if (($rule = findRule($name, $rules)) !== false && shouldUpdate($name, $info['hash'])) {
      ...
      curl_setopt_array($curl, array(
        CURLOPT_URL=>'https://img-pc.so-net.tw/dl/pool/AssetBundles/'.substr($info['hash'],0,2).'/'.$info['hash'],
      ));
...
function checkSoundResource($manifest, $rules, $version) {
  ...
  foreach ($manifest as $name => $info) {
    if (($rule = findRule($name, $rules)) !== false && shouldUpdate($name, $info['hash'])) {
      _log('download '. $name.' '.$info['hash']);
      curl_setopt_array($curl, array(
        CURLOPT_URL=>'https://img-pc.so-net.tw/dl/pool/Sound/'.substr($info['hash'],0,2).'/'.$info['hash'],
      ));
      ...
      if ($info['hasAwb']) {
        ...
        curl_setopt_array($curl, array(
          CURLOPT_URL=>'https://img-pc.so-net.tw/dl/pool/Sound/'.substr($awbInfo['hash'],0,2).'/'.$awbInfo['hash'],
        ));
...
function checkMovieResource($manifest, $rules, $version) {
  ...
  foreach ($manifest as $name => $info) {
    if (($rule = findRule($name, $rules)) !== false && shouldUpdate($name, $info['hash'])) {
      ...
      curl_setopt_array($curl, array(
        CURLOPT_URL=>'https://img-pc.so-net.tw/dl/pool/Movie/'.substr($info['hash'],0,2).'/'.$info['hash'],
...
function checkAndUpdateResource($TruthVersion) {
  ...
  foreach ($resourceToExport as $name=>$rules) {
    ...
    if (isset($manifest[$name]) && shouldUpdate($name, $manifest[$name]['hash'])) {
      curl_setopt_array($curl, array(
        CURLOPT_URL=>'https://img-pc.so-net.tw/dl/Resources/'.$TruthVersion.'/Jpn/AssetBundles/iOS/'.$name,
      ));
  ...
  do {
    curl_setopt_array($curl, array(
      CURLOPT_URL=>'https://img-pc.so-net.tw/dl/Resources/'.$TruthVersion.'/Jpn/Sound/manifest/sound2manifest',
    ));
    $submanifest = curl_exec($curl);
    $submanifest = parseManifest($submanifest);
  ...
  do {
    ...
    curl_setopt_array($curl, array(
      CURLOPT_URL=>'https://img-pc.so-net.tw/dl/Resources/'.$TruthVersion.'/Jpn/Movie/PC/High/'.$name,
    ));
    $submanifest = curl_exec($curl);
    $submanifest = parseManifest($submanifest);
